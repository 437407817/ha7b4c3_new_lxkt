...
static char Line[256];				/* Console input buffer */
static BYTE Buff[4096] __attribute__ ((aligned (4))) ;	/* Working buffer */

static FATFS FatFs;				/* File system object for each logical drive */
static IIC_FIL File[2];				/* File objects */
...
uint32_t COMMON_BufferRead(uint8_t* pBuffer, uint8_t ReadAddr, uint16_t NumByteToRead,uint8_t mod){

	
	switch (mod){
		case 0:I2C_EE_BufferRead( pBuffer,  ReadAddr,  NumByteToRead);break;
		case 1:SPI_FLASH_BufferRead( pBuffer,  ReadAddr,  NumByteToRead);
//		SYSTEM_DEBUG("ReadAddr =%d NumByteToRead =%d ",ReadAddr,NumByteToRead);
//		SYSTEM_DEBUG_ARRAY_MESSAGE(pBuffer,NumByteToRead,"COMMON_BufferRead");
		break;;
//		case 2:W25Q256_Read( pBuffer,  ReadAddr,  NumByteToRead);break;;
		default:break;
	}
	return 0;
}

uint32_t COMMON_BufferWrite(uint8_t* pBuffer, uint8_t ReadAddr, uint16_t NumByteToRead,uint8_t mod){

	
	switch (mod){
		case 0:I2C_EE_BufferWrite( pBuffer,  ReadAddr,  NumByteToRead);break;
		case 1:SPI_FLASH_BufferWrite( pBuffer,  ReadAddr,  NumByteToRead);break;
		
//		case 2:SPI_FLASH_WriteMultiBytes( pBuffer,  ReadAddr,  NumByteToRead);break;;
//		case 3:SPI_FLASH_BufferWrite( pBuffer,  ReadAddr,  NumByteToRead);break;;
		default:break;
	}
}


FRESULT	COMMON_init(uint8_t mod){

	
		switch (mod){
		case 0:I2C_EE_Hard_Init();break;
		case 1:SPI_FLASH_Init();break;;
		default:break;
	}
...
" fi <ld#> [<mount>]- Force initialized the volume. -> first-{:fi} -> second-{:fi 1}\n"//++++
	" fq <pd#>- umount the volume {:fq 1}\n"
	" fs [<path>] - Show volume status\n"
	" fl [<path>] - Show a directory\n"
	" fo <mode> <file> - Open a file  {:fo 7 new.txt}\n"
	" fc - Close the file\n"
	" fe <position> - Move fp in normal seek  {:fe 0}\n"//+++++++++
	" fd[r] <len> - Read and dump the file and increase read point,([r]no increase) {:fd 16}\n"//+++++++++
...
//				if (!xatoi(&ptr, &p2)) p2 = 0;
				...
				xprintf("f_init=%d \n",p1);
//			  put_rc(f_mount(&FatFs, path, (BYTE)p2));
				break;
...
//**************************************************************	START	*******************************************
			...
			case 'd' :	/* fd <len> - read and dump file from current fp */
				switch (*ptr++) {
				case 'r': p2 = 1; break;/* fdr <len> - not increase read point */
				default: p2 = 0;
				}
				if (!xatoi(&ptr, &p1)) break;
				
				File[0].fposition = File[0].fptr;
				while (p1) {
					if ((UINT)p1 >= 16) { File[0].cnt = 16; p1 -= 16; }
					else 				{ File[0].cnt = p1; p1 = 0; }
					//res = f_read(&File[0], Buff, cnt, &cnt);//uint32_t sEE_ReadBuffer(uint8_t* pBuffer, uint16_t ReadAddr, uint16_t* NumByteToRead)
					File[0].remainingQuantity=File[0].cnt;
//-					res = sEE_ReadBuffer(Buff, File[0].fposition, &File[0].remainingQuantity);//File[0].remainingQuantity
					COMMON_BufferRead(Buff, File[0].fposition, File[0].remainingQuantity,mod);
//-					if (res != FR_OK) { put_rc(res); break;}
//-					if (File[0].remainingQuantity) break;
					put_dump(Buff, File[0].fposition, File[0].cnt, 1);
					File[0].fposition += 16;
				}if(p2 ==0){File[0].fptr=File[0].fposition;}
				break;
			...
			case 'p' :	
				switch (*ptr++) 
					...
					case 'c':while (*ptr == ' ') ptr++;p1=1;//if (!xatoi(&ptr, &p1)) break;
						COMMON_BufferWrite((uint8_t *)ptr, File[0].fptr, p1,mod);
						xprintf("put_rc:%c \n",*ptr);
						;break;
					case 's':while (*ptr == ' ') ptr++;
						p1=0;
						while (*ptr) {Buff[p1++]=*ptr++;}	
//						sEE_WriteBuffer(&Buff[0],File[0].fptr,p1);
						COMMON_BufferWrite(&Buff[0], File[0].fptr, p1,mod);
						Buff[p1]='\0';File[0].fptr+=p1;
						//while (*ptr) {sEE_WriteBuffer((uint8_t *)ptr++,File[0].fptr++,p1);}	
						xprintf("write: %s   num: %d\n",Buff,p1);
						break;
					case 'a':if (!xatoi(&ptr, &p1)) break;
						if ((uint16_t)p1 >= 0xff) { p1=0xff; };
						if (!xatoi(&ptr, &p2)) break;
						//xprintf("rc=%d  put_rc:%c \n",sEE_WriteBuffer((uint8_t *)ptr,File[0].fptr,p1),*ptr);break;
						p3=0;
						while (p2) {Buff[p3++]=p1;p2--;}	
						p1=p3;
						//sEE_WriteBuffer(&Buff[0],File[0].fptr,p3);
					
						//while (*ptr) {sEE_WriteBuffer((uint8_t *)ptr++,File[0].fptr++,p1);}	
						COMMON_BufferWrite(&Buff[0],File[0].fptr,p3,mod);
						xprintf("num: %d\n",p1);
						break;						
...
//				f_close(&File[1]);
			...
			case 'g' :	/* fg <path> - Change current directory */
				...
				break;
#if FF_FS_RPATH >= 2
			case 'f' :	/* ff - Show current dir path */
