...
/**
 * @brief  软件方式强制恢复I2C总线（针对STM32H7x3常见死锁）
 * @param  hi2c: I2C句柄
 * @retval HAL_StatusTypeDef
 */
HAL_StatusTypeDef I2C_Recover_Bus_Stuck(I2C_HandleTypeDef *hi2c)
{
    GPIO_InitTypeDef GPIO_InitStruct = {0};
    uint32_t scl_pin  = GTP_I2C_SCL_PIN;   // 例如 GPIO_PIN_6
    uint32_t sda_pin  = GTP_I2C_SDA_PIN;   // 例如 GPIO_PIN_7
    GPIO_TypeDef *scl_port = GTP_I2C_SCL_GPIO_PORT;  // 例如 GPIOB
    GPIO_TypeDef *sda_port = GTP_I2C_SDA_GPIO_PORT;

    // 1. 先解锁HAL状态机
    __HAL_UNLOCK(hi2c);

    // 2. 关闭I2C外设（重要！H7必须彻底关掉）
    __HAL_I2C_DISABLE(hi2c);

    // 3. 把SCL/SDA临时切换成GPIO开漏输出
    GPIO_InitStruct.Pin  = scl_pin | sda_pin;
    GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_OD;
    GPIO_InitStruct.Pull = GPIO_NOPULL;
    GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_VERY_HIGH;
    HAL_GPIO_Init(scl_port, &GPIO_InitStruct);  // SCL/SDA都初始化
    ...
    // 4. 强制产生9~10个SCL时钟脉冲（最有效清从机状态）
    for (uint8_t i = 0; i < 10; i++)
    {
        HAL_GPIO_WritePin(scl_port, scl_pin, GPIO_PIN_RESET);
        HAL_Delay(1);   // ~100us ~ 500us，根据从机手册调整
        HAL_GPIO_WritePin(scl_port, scl_pin, GPIO_PIN_SET);
        HAL_Delay(1);
    }

    // 5. 再强制发一个STOP条件（SDA高→低→高，SCL保持高）
    HAL_GPIO_WritePin(sda_port, sda_pin, GPIO_PIN_RESET);
    HAL_Delay(1);
    HAL_GPIO_WritePin(scl_port, scl_pin, GPIO_PIN_SET);   // SCL已高
    HAL_Delay(1);
    HAL_GPIO_WritePin(sda_port, sda_pin, GPIO_PIN_SET);
    HAL_Delay(2);
    ...
    GPIO_InitStruct.Pull      = GPIO_PULLUP;          // H7建议用内部上拉或外部4.7k
    GPIO_InitStruct.Speed     = GPIO_SPEED_FREQ_VERY_HIGH;     // 或 GPIO_SPEED_FREQ_VERY_HIGH
    GPIO_InitStruct.Alternate = GTP_I2C_SCL_AF; // 你的AF值，例如 GPIO_AF4_I2C1
    HAL_GPIO_Init(scl_port, &GPIO_InitStruct);
    HAL_GPIO_Init(sda_port, &GPIO_InitStruct);

    // 7. 软件复位I2C外设（H7推荐）
    GTP_RCC_II2C_FORCE_RESET();   // 根据你的I2C实例改 I2C1/I2C2/I2C3...
