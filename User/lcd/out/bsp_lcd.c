...
//  ili9325_DrawHLine,
//  ili9325_DrawVLine,
//  LCD_GetLcdPixelWidth,
//  LCD_GetLcdPixelHeight,
//  ili9325_DrawBitmap,
...
//  
//  if(LCD_RGB888_drv.ReadID() == 0)
//  {
//    LcdDrv = &LCD_RGB888_drv;
...
void LCD_WritePixel(uint16_t Xpos, uint16_t Ypos, uint32_t RGBCode)
{ 
	LCD_SetXSize(Xpos);
  LCD_SetYSize(Ypos);
  /* 清屏 */ 
  LL_FillBuffer(ActiveLayer, (uint32_t *)(Ltdc_Handler.LayerCfg[ActiveLayer].FBStartAdress), LCD_GetXSize(), LCD_GetYSize(), 0, RGBCode);
}

/**
  * @brief  按起始/结束坐标填充矩形（DMA2D加速）
  * @param  StartXpos: 矩形左上角X坐标（如10）
  * @param  StartYpos: 矩形左上角Y坐标（如10）
  * @param  EndXpos:   矩形右下角X坐标（如100）
  * @param  EndYpos:   矩形右下角Y坐标（如80）
  * @param  Color:     填充颜色（需匹配当前LCD像素格式，如RGB565=0x00FF00）
  * @retval None
  */
void LCD_FillRect_ByColor(uint16_t StartXpos, uint16_t StartYpos, uint16_t EndXpos, uint16_t EndYpos, uint32_t Color)
{
  uint32_t x_address = 0;
  uint32_t Width = 0;
  uint32_t Height = 0;
  
  /* 1. 坐标合法性修正（避免传参顺序错误/超出屏幕范围） */
  // 交换起始/结束坐标，确保 Start <= End
  if (StartXpos > EndXpos) { uint16_t temp = StartXpos; StartXpos = EndXpos; EndXpos = temp; }
  if (StartYpos > EndYpos) { uint16_t temp = StartYpos; StartYpos = EndYpos; EndYpos = temp; }
  
  // 裁剪到屏幕有效范围（避免帧缓冲越界）
  StartXpos = (StartXpos > LCD_GetXSize() - 1) ? (LCD_GetXSize() - 1) : StartXpos;
  StartYpos = (StartYpos > LCD_GetYSize() - 1) ? (LCD_GetYSize() - 1) : StartYpos;
  EndXpos   = (EndXpos > LCD_GetXSize() - 1) ? (LCD_GetXSize() - 1) : EndXpos;
  EndYpos   = (EndYpos > LCD_GetYSize() - 1) ? (LCD_GetYSize() - 1) : EndYpos;
  
  // 计算矩形实际宽高（+1是因为包含起始和结束坐标）
  Width  = EndXpos - StartXpos + 1;
  Height = EndYpos - StartYpos + 1;
  
  // 宽高为0时直接返回（无效矩形）
  if (Width == 0 || Height == 0) return;

  /* 2. 根据像素格式计算帧缓冲起始地址（复用原有逻辑） */
  if (Ltdc_Handler.LayerCfg[ActiveLayer].PixelFormat == LTDC_PIXEL_FORMAT_ARGB8888)
  {
    x_address = Ltdc_Handler.LayerCfg[ActiveLayer].FBStartAdress + 4 * (LCD_GetXSize() * StartYpos + StartXpos);
  }
  else if (Ltdc_Handler.LayerCfg[ActiveLayer].PixelFormat == LTDC_PIXEL_FORMAT_RGB888)
  {
    x_address = Ltdc_Handler.LayerCfg[ActiveLayer].FBStartAdress + 3 * (LCD_GetXSize() * StartYpos + StartXpos);
  }
  ...
  {
    x_address = Ltdc_Handler.LayerCfg[ActiveLayer].FBStartAdress + 2 * (LCD_GetXSize() * StartYpos + StartXpos);
  }
  else
  {
    // 默认按2字节像素格式处理（兼容未定义格式）
    x_address = Ltdc_Handler.LayerCfg[ActiveLayer].FBStartAdress + 2 * (LCD_GetXSize() * StartYpos + StartXpos);
  }

  /* 3. 调用DMA2D填充（复用原有LL_FillBuffer，直接传入指定颜色） */
  LL_FillBuffer(ActiveLayer, (uint32_t *)x_address, Width, Height, (LCD_GetXSize() - Width), Color);
}
///**
//  * @brief  Read pixel.
//  * @param  None
...
{
 return (uint16_t)lcd_param[0].lcd_pixel_width;
}
...
{
 return (uint16_t)lcd_param[0].lcd_pixel_height;
}
