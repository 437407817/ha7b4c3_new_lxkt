...
void LCD_FillRect_Color2(int16_t x1, int16_t y1, int16_t x2, int16_t y2, uint8_t *color)
{
    // 边界检查（防止越界）
    if(x1 > x2 || y1 > y2 || x2 >= LCD_WIDTH || y2 >= LCD_HEIGHT) return;
    if(x1 < 0) x1 = 0;
    if(y1 < 0) y1 = 0;

    uint32_t dst_addr = 0;
    // 显存起始地址计算：基地址 + 偏移（y1行×宽度 + x1列）× 像素字节数
    dst_addr = Ltdc_Handler.LayerCfg[ActiveLayer].FBStartAdress + PIXEL_BYTES*(LCD_WIDTH*y1 + x1);

    /* DMA2D配置 */
    DMA2D->CR      = 0x00000000UL; // 内存到内存模式
    DMA2D->FGMAR   = (uint32_t)color; // 源地址（颜色缓存）
    DMA2D->OMAR    = (uint32_t)dst_addr; // 目标地址（显存）
    DMA2D->FGOR    = 0; // 源行偏移（单像素无偏移）
    DMA2D->OOR     = (LCD_WIDTH - (x2 - x1 +1)); // 目标行偏移 = 屏幕宽度 - 矩形宽度
    DMA2D->FGPFCCR = LCD_PIXFORMAT;  // 像素格式
    DMA2D->NLR     = (uint32_t)((x2 - x1 + 1) << 16) | (uint16_t)(y2 - y1 + 1); // 宽、高
...
//    uint32_t offline;
    ...
    /* 1. 校验坐标有效性（确保在LCD显示范围内） */
    if (sx > ex || sy > ey || ex >= LCD_GetXSize() || ey >= LCD_GetYSize()) {
        return ; // 坐标无效
    }
    __HAL_RCC_DMA2D_CLK_ENABLE();     /* 使能DM2D时钟 */
    /* 2. 计算填充区域尺寸和偏移量 */
    width = ex - sx + 1;    // 宽度（像素数）
    height = ey - sy + 1;   // 高度（像素数）
//    offline = LCD_WIDTH - width; // 行偏移（每行结束到下一行开始的像素数）

    /* 3. 计算目标地址（帧缓冲中起始像素的地址） */
    dst_addr = Ltdc_Handler.LayerCfg[ActiveLayer].FBStartAdress + 
               PIXEL_BYTES * (LCD_WIDTH * sy + sx);

    /* 4. 配置DMA2D句柄 */
    Dma2d_Handler.Instance = DMA2D;
    Dma2d_Handler.Init.Mode = DMA2D_M2M; // 内存到内存模式
    Dma2d_Handler.Init.OutputOffset = (LCD_WIDTH - (ex - sx +1));             /* 设置行偏移 */ // 输出偏移
...
//    }
  ...
  if(HAL_DMA2D_Init(&Dma2d_Handler) == HAL_OK) 
  {
    ...
    /* 8. 配置Layer1（F4 HAL库方式：先填充句柄的LayerCfg数组） */
//    Dma2d_Handler.LayerCfg[1].AlphaMode = DMA2D_NO_MODIF_ALPHA;    // 不修改Alpha
//    Dma2d_Handler.LayerCfg[1].InputAlpha = 0xFF;                   // 全局不透明
//    Dma2d_Handler.LayerCfg[1].InputColorMode = DMA2D_ARGB8888;     // 源格式=ARGB8888
//    Dma2d_Handler.LayerCfg[1].InputOffset = 0;                     // 源数据无行偏移

    if(HAL_DMA2D_ConfigLayer(&Dma2d_Handler, 1) == HAL_OK) 
    {
			/* 6. 启动DMA2D传输（内存到内存，填充指定区域） */
      if (HAL_DMA2D_Start(&Dma2d_Handler, (uint32_t)p_address_color, dst_addr, width, height) == HAL_OK)
      {
        /* DMA轮询传输*/  
    ...
    /* 6. 启动DMA2D传输（内存到内存，填充指定区域） */
//    status = HAL_DMA2D_Start(&Dma2d_Handler, (uint32_t)color, dst_addr, width, height);
//    if (status != HAL_OK) {
//        return ;
//    }

//    /* 7. 等待传输完成（超时时间设置为50ms） */
//    status = HAL_DMA2D_PollForTransfer(&Dma2d_Handler, 50);
//    if (status != HAL_OK) {
//        // 传输超时，可根据需要添加错误处理
//        HAL_DMA2D_Abort(&Dma2d_Handler);
//        return ;
//    }

    /* 8. 关闭DMA2D（可选，根据系统需求决定是否保持初始化状态） */
//    HAL_DMA2D_DeInit(&Dma2d_Handler);
