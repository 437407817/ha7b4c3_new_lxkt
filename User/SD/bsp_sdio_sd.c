...
//  }
  ...
  if(SD_state == MSD_OK)
  {
    ...
    {
      SD_state = MSD_ERROR;
					printf(" 1ppSD_state %d %d\r\n",SD_state,uSdHandle.ErrorCode);
...
void SDMMC_PrintCardInfo(void)
{

	SD_HandleTypeDef *phsd = &uSdHandle;
	
	printf("CardVolume:\t%.2fGB\r\n", (float)(phsd->SdCard.BlockNbr >> 1) / 1024 / 1024);
...
uint8_t BSP_SD_IsDetected(void)
{
  // 这里假设检测引脚的逻辑，实际需要根据硬件连接修改
  __IO uint8_t status = SD_PRESENT;
  // 假设检测引脚连接到某个 GPIO 引脚，这里简单示例
