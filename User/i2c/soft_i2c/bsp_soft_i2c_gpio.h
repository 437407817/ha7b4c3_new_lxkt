#ifndef _BSP_SOFT_I2C_GPIO_H
#define _BSP_SOFT_I2C_GPIO_H
...
#define EEPROM_RCC_I2C_PORT 	RCC_APB2Periph_GPIOB | RCC_APB2Periph_GPIOB		/* GPIO端口时钟 */
...
//	#define EEPROM_I2C_SCL_0()  EEPROM_GPIO_PORT_I2C->BRR = EEPROM_I2C_SCL_PIN				/* SCL = 0 */
	// STM32H743版本 - 推荐：统一使用BSRR寄存器（低16位置位，高16位复位）
	#define EEPROM_I2C_SCL_1()  EEPROM_GPIO_PORT_I2C->BSRR = EEPROM_I2C_SCL_PIN;        /* SCL = 1（置位，输出高） */
	#define EEPROM_I2C_SCL_0()  EEPROM_GPIO_PORT_I2C->BSRR = (uint32_t)EEPROM_I2C_SCL_PIN << 16;  /* SCL = 0（复位，输出低） */
	
	#define EEPROM_I2C_SDA_1()  EEPROM_GPIO_PORT_I2C->BSRR = EEPROM_I2C_SDA_PIN				/* SDA = 1 */
	#define EEPROM_I2C_SDA_0()  EEPROM_GPIO_PORT_I2C->BSRR = (uint32_t)EEPROM_I2C_SDA_PIN	<< 16;			/* SDA = 0 */
	
	// STM32H743优化版 - 显式类型转换，更严谨
#define EEPROM_I2C_SDA_READ()  (((uint32_t)EEPROM_GPIO_PORT_I2C->IDR & (uint32_t)EEPROM_I2C_SDA_PIN) != 0U)  /* 读SDA口线状态 */	/* 读SDA口线状态 */
#endif

void i2c_Soft_CfgGpio(void);
void i2c_Start(void);
void i2c_Stop(void);
