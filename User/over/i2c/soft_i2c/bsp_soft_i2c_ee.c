...
#if defined (SIM_SINGLE_BYTE_ADDRESS) 
	/* 第4步：发送字节地址，24C02只有256字节，因此1个字节就够了，如果是24C04以上，那么此处需要连发多个地址 */
	i2c_SendByte((uint8_t) _usAddress );
	
#elif	 defined (SIM_DOUBLE_BYTE_ADDRESS) 
	
	i2c_SendByte((uint8_t)((_usAddress & 0xFF00)>>8));
		/* 第5步：等待ACK */
	if (i2c_WaitAck() != 0)
	{
		goto cmd_fail;	/* EEPROM器件无应答 */
...
uint8_t ee_WriteBytes(uint8_t *_pWriteBuf, uint16_t _usAddress, uint16_t _usSize)
{
	...
	*/
	...
	i2c_SendByte((uint8_t)((usAddr & 0xFF00)>>8));
		/* 第5步：等待ACK */
	if (i2c_WaitAck() != 0)
	{
		goto cmd_fail;	/* EEPROM器件无应答 */
...
void ee_Erase(void)
{
	...
	{
		SYSTEM_ERROR("擦除eeprom出错！\r\n");
		return;
	}
	else
	{
		SYSTEM_ERROR("擦除eeprom成功！\r\n");
	}
}
